...
for model in model_names:
    df[f'Error {model}'] = (df[f'Pred. {model}'] - df['Demanda'])*100/df['Demanda']
...
metrics_dict = {name: metrics(df_specific_day[f'Pred. {name}'], df_specific_day['Demanda']) for name in model_names}
metrics_df = pd.DataFrame.from_dict(metrics_dict).transpose()

st.write(f'Predicción de Demanda {day} diciembre 2017')
st.line_chart(df_specific_day, x='Hora', y=['Demanda', 'Pred. M1', 'Pred. M2', 'Pred. M3'], y_label='MW', color=colors)
...
with left_column:
    ...
    st.bar_chart(df_specific_day, x='Hora', y=['Error M1', 'Error M2', 'Error M3'], color=colors[1:4], y_label='%')
